...
func (p *Personnage) CreateCharacter() {
	...
	ClearConsole()
	p.Initialize(nom, classe, 5, 0, hpMax, hpMax, []Item{{"Argent", 300}, {"Potions", 3}}, spells)
...
func (p *Personnage) Initialize(nom string, classe string, niveau int, ennemi int, hp int, hpMax int, inventaire []Item, skill []Spell) {
	p.nom = nom
	p.classe = classe
	p.niveau = niveau
	p.ennemi = ennemi
